...
!-----------------------------------------------
      USE prsrcn_I 
      USE parsjl_I 
!jbq pack_I commented out - replace PACK by
!jbq  integers with KIND=   or MVBITS, whichever is faster
      USE pack_I
!GG   USE pack23
!jbq
      USE convrt_I 
...
!
      IF (JB /= LOADALL) THEN 
         WRITE (6, *) 'Loading CSL File for block ', JB 
      ELSE 
         WRITE (6, *) 'Loading CSL File for ALL blocks ' 
      ENDIF 
...
!
         ...
         ELSE IF (RECL == '-') THEN 
            ISPARC = -1 
         ELSE 
            WRITE (ISTDE, *) MYNAME//': Incorrect parity ', 'specification;' 
            GO TO 26 
         ENDIF 
         LOC = LOC - 1 
!
