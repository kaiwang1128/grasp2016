...
!-----------------------------------------------
      ...
      REAL(DOUBLE) , INTENT(IN) :: CFROM 
      REAL(DOUBLE) , INTENT(IN) :: CTO 
      CHARACTER  :: TYPE 
...
!
      ...
      ELSE IF (ITYPE >= 4) THEN 
         IF (KL<0 .OR. KL>MAX(M-1,0)) THEN 
            INFO = -2 
         ELSE IF (KU<0 .OR. KU>MAX(N-1,0) .OR. (ITYPE==4 .OR. ITYPE==5) .AND. &
               KL/=KU) THEN 
            INFO = -3 
         ELSE IF (ITYPE==4 .AND. LDA<KL+1 .OR. ITYPE==5 .AND. LDA<KU+1 .OR. &
...
!
         DO J = 1, N 
            A(:MIN(J+1,M),J) = A(:MIN(J+1,M),J)*MUL 
         END DO 
!
      CASE (4)  
!
...
!
         ...
         K4 = N + 1 
         DO J = 1, N 
            A(:MIN(K3,K4-J),J) = A(:MIN(K3,K4-J),J)*MUL 
         END DO 
!
      CASE (5)  
!
