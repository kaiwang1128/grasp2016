...
!-----------------------------------------------
      ...
      REAL(DOUBLE) , INTENT(IN) :: PIVMIN 
      INTEGER , INTENT(INOUT) :: NVAL(*) 
      INTEGER , INTENT(INOUT) :: NAB(MMAX,*) 
      INTEGER , INTENT(INOUT) :: IWORK(*) 
      REAL(DOUBLE) , INTENT(IN) :: D(*) 
      REAL(DOUBLE)  :: E(*) 
      REAL(DOUBLE) , INTENT(IN) :: E2(*) 
      REAL(DOUBLE) , INTENT(INOUT) :: AB(MMAX,*) 
...
!
      IF (IJOB == 2) THEN 
         C(:MINP) = HALF*(AB(:MINP,1)+AB(:MINP,2)) 
      ENDIF 
!
!     Iteration loop
!
...
!                      w   s.t. N(w) = NVAL
!
               WHERE (IWORK(KF:KL) <= NVAL(KF:KL))  
                  AB(KF:KL,1) = C(KF:KL) 
                  NAB(KF:KL,1) = IWORK(KF:KL) 
               END WHERE 
               WHERE (IWORK(KF:KL) >= NVAL(KF:KL))  
                  AB(KF:KL,2) = C(KF:KL) 
                  NAB(KF:KL,2) = IWORK(KF:KL) 
               END WHERE 
            ENDIF 
!
         ELSE 
...
!
         ...
         IF (IJOB == 3) THEN 
            DO JI = KF, KL 
               ...
               IF (TMP1>=MAX(MAX(ABSTOL,PIVMIN),RELTOL*TMP2) .AND. NAB(JI,1)<&
                  NAB(JI,2)) CYCLE  
!
...
!        Choose Midpoints
!
         C(KF:KL) = HALF*(AB(KF:KL,1)+AB(KF:KL,2)) 
!
