...
!
         IF (LSAME(UPLO,'U')) THEN 
            ...
            ELSE 
               ...
               DO J = 1, N 
                  TEMP = X(JX) 
                  IX = KX 
                  TEMP = TEMP - SUM(A(:J-1,J)*X(IX:(J-2)*INCX+IX:INCX)) 
                  IF (NOUNIT) TEMP = TEMP/A(J,J) 
                  X(JX) = TEMP 
                  JX = JX + INCX 
               END DO 
         ...
         ELSE 
            IF (INCX == 1) THEN 
               DO J = N, 1, -1 
                  TEMP = X(J) 
                  TEMP = TEMP - SUM(A(N:1+J:(-1),J)*X(N:1+J:(-1))) 
                  IF (NOUNIT) TEMP = TEMP/A(J,J) 
                  X(J) = TEMP 
               END DO 
            ELSE 
               ...
               DO J = N, 1, -1 
                  TEMP = X(JX) 
                  IX = KX 
                  TEMP = TEMP - SUM(A(N:1+J:(-1),J)*X(IX:INCX*(J-N+1)+IX:(-INCX&
                     ))) 
                  IF (NOUNIT) TEMP = TEMP/A(J,J) 
                  X(JX) = TEMP 
                  JX = JX - INCX 
