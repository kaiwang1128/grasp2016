...
!
        ...
        DO I3=IP1,IG1,2
          ...
          IF(IREZ.EQ.2)IFAZ=J(1)+J(4)-2*J12+2*KRA
        ...
        IF (ICOLBREI .EQ. 1) THEN
          ...
          IF(IREZ.EQ.1)CALL SPEAK(JJA,JJB,IA,IB,IC,ID,KRA,BB)
          IF(IREZ.EQ.2)CALL SPEAK(JJA,JJB,IC,ID,IA,IB,KRA,BB)
...
!
        ...
        DO I3=IP1,IG1,2
          ...
          IF(IREZ.EQ.2)IFAZ=J(1)+2*J(2)+J(4)+4*J12+2*KRA
          IF((IFAZ/4)*4.NE.IFAZ)AA=-AA
          AB=AB+AA
        END DO
        IF(DABS(AB).LT.EPS) CYCLE
        ...
        IF (ICOLBREI .EQ. 1) THEN
          BB=A1*AB*DBLE(IFAZFRCS)
          IF(IREZ.EQ.1)CALL SPEAK(JJA,JJB,IA,IB,ID,IC,KRA,BB)
          IF(IREZ.EQ.2)CALL SPEAK(JJA,JJB,IC,ID,IB,IA,KRA,BB)
        ELSE IF (ICOLBREI .EQ. 2) THEN
          NU=KRA
