...
!   D u m m y   A r g u m e n t s
!-----------------------------------------------
      integer, intent(out)      :: ioutC,ioutj
...
!GG      allocate(Jcoup(1:NNNW,1:NCF))
...
!-----------------------------------------------
!GG NIST
      integer :: i, j, jj, ii, string_l, IBLKNUM,ioutC,ioutj
...
!-----------------------------------------------
      ...
      call inscreen(THRESH,levels,number_of_levels,ioutC,ioutj)
...
!GG NIST
            ...
            if(IMINCOMPOFF == 1) THEN
               if (level > 1 .and. (wb*100.+dabs(MINCOMP)) <= 100.) THEN
                  WRITE (*,"(A,F5.0,A)")   &
                    "Attenshen!!!  The program not reach the accuracy",MINCOMP," %"
                  print *, "level, sum of weights in LSJ = ",level,wb
                  print *,""
               end if
            else
               if (level > 1 .and. abs(wb) > 1.001) then
                  print*,"Attenshen!!!  The sum of weights is bigger then 1.0001"
                  print *, "level, sum of weights in LSJ = ",level,wb
               end if
               if (level > 1 .and.  abs(wb) < .998) then
                  print*,"Attenshen!!!  The sum of weights is little then .998"
                  print *, "level, sum of weights in LSJ = ",level,wb
               end if
            end if
...
!GG      allocate(occupation(NNNW))
!GG      allocate(string_J(NNNW))
!GG      allocate(string_X(NNNW))
      do j = NCORE+1, NW
...
!GG      allocate(shell_temp(NNNW));  allocate(nlLSval(NNNW));  allocate(nljjval(NNNW))
      nljjval = 0;  nlLSval = 0
!
