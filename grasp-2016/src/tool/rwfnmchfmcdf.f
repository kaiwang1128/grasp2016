...
**************************************************************
      ...
      integer max(nwf),nl(nwf),l(nwf),rdnum,rdorb
...
c u  *******************************************************************
c u  write out initial orbitals Pnl and Qnl as input
c u  for Grant's MCDF program
c u  *******************************************************************
*
      Z=0.0
      do 1 i=1,nwf
...
C
C **** read THE NL- and THE L-VALUES
C
         il=1
         call skip (il,line)
         nl(i)=rdnum(il,line)
...
C  *****  construct MCHF grid:
C  *****  GENERATE ARRAYS FOR R WITH A CONSTANT MESH
...
C
C read AN integer FROM line
C
      integer rdnum
      character*3 line
 
C ICHAR('I')-ICHAR('0') RETURNS THE integer VALUE OF A character
C ICHAR('0')=48 or 33 DEPendENT ON THE character SET USED, BUT then
C ICHAR('3')=51 or 36, THE FINAL ANSWER BEING CorRECT
...
C
C converts character to orbital angular momentum
C
      integer rdorb
