...
!                                Corrected Feb 2017
! Modified for symbolic CSF lists, May 2017
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
integer            :: ncore6to5,nmaxcan

character(len=1)   :: lstring,ans
character(len=2)   :: sel(15),nstring
...
! For symbolic, generate clist.ref
...
write(*,*) 
write(*,*) 'RCSFGENERATE_SYM'
write(*,*) 'This program generates a list of symbolic CSFs'
write(*,*) 
...
if (ntimes.eq.1) then
  write(*,*) 'Generation of general CSF space built on canonical orbitals'
else
  write(*,*) 'Generation of symbolic CSF space built on symmetry ordered orbitals'
end if 
...
! Loop over remaining orbitals in the orbital set
   ...
   if (nmax.gt.maxval(n)) then
      do nr = maxval(n)+1,nmax
         do lr = 0,min(nr-1,lmax)
            if (ntimes.gt.1) then                              ! Orbitals for the symbolic part
...
! Write symmetry orbitals to clist.ref

do j = 0,10
  do i = max(nmaxcan+1,j+1),15
    write (nstring,'(i2)') i
    if (j.eq.0) then
      write(113,'(a)') trim(adjustl(nstring))//'s'
    elseif (j.eq.1) then
