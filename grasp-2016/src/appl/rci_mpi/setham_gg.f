...
*
     ...
     &  NCOREtmp, NVPItmp, NKEItmp, NVINTItmp, NELMNTtmp, ncftmp
      integer*8 nelmnt,NELMNTtmp,nelmntt     ! AK: sizez of all these variables must be the same
*
!-----------------------------------------------------------------------
cAK Handling the -i8 option of ifort and -fdefault-integer-8 option of gfortran
cff   Set default integer lenght to 4
      ISIZE = 4
c
c      PRINT *, 'Calling setham ...'
...
!     &             , '  block = ', jblock
         if (mod(ic-1,100).eq.0) then
            PRINT '(A4,I10,A9,I10,A9,I5,A8,I5)', 'Row ', ic,
     &              ' nnonz = ', nelc,
