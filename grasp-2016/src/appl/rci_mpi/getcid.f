...
cff   Set the default integer length to 4
      ...
      IF (ISIZE.EQ.8) THEN
        I_MPI = MPI_INTEGER8
        L_MPI = MPI_LOGICAL8     !! This needs to be checked.
      ENDIF
c
...
! Quantities to be obtained: LFORDR
     ...
     &,                    ' as first-order perturbations?'
            LFORDR = GETYN ()
         ENDIF
         CALL MPI_Bcast (LFORDR,1,L_MPI,0,MPI_COMM_WORLD,ierr)
      ELSE
         LFORDR = .FALSE.
      ENDIF
...
! Get iccutblk() from the user-input
  ...
  123          READ (istdi,*) ntmp
         ...
         CALL MPI_Bcast (iccutblk, nblock, I_MPI, 0,
     &                     MPI_COMM_WORLD, ierr)
      ENDIF
...
!           YES = GETYN ()
!        ENDIF
!        CALL MPI_Bcast (YES, 1, L_MPI, 0, MPI_COMM_WORLD, ierr)
...
!              READ *, COEFFCUT2
!           ENDIF
!           CALL MPI_Bcast (NCSFPRE, 1, I_MPI, 0
!    &,                              MPI_COMM_WORLD, ierr)
...
! Quantities to be obtained: LTRANS, WFACT
      ...
      CALL MPI_Bcast (LTRANS, 1, L_MPI, 0, MPI_COMM_WORLD, ierr)
...
! Other interactions ? One logical for each case. Done altogether
      ...
      IF (myid .EQ. 0) THEN
         ...
         IF (LSE.EQV..TRUE.) THEN
            NQEDCUT = 1
            WRITE (istde,*) 'Largest n quantum number ',
     :        'for including self-energy for orbital'
      ...
      CALL MPI_Bcast (LVP, 1, L_MPI, 0, MPI_COMM_WORLD, ierr)
      CALL MPI_Bcast (LNMS, 1, L_MPI, 0, MPI_COMM_WORLD, ierr)
      CALL MPI_Bcast (LSMS, 1, L_MPI, 0, MPI_COMM_WORLD, ierr)
...
!         ...To prevent subsequent BCAST when YES is false
     ...
     &                          MPI_COMM_WORLD, ierr)
            CALL MPI_Bcast (N, 1, I_MPI, 0, MPI_COMM_WORLD, ierr)
         ENDIF
      ENDIF
*
