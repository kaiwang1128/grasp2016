...
!-----------------------------------------------------------------------
      ...
      CALL MPI_Reduce (NCOEItmp, NCOEI_a, 1, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)
      CALL MPI_Reduce (NCOECtmp, NCOEC_a, 1, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)
      CALL MPI_Reduce (NCTEItmp, NCTEI_a, 1, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)
      CALL MPI_Reduce (NCTECtmp, NCTEC_a, 1, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)

      IF (LTRANS) THEN
         CALL MPI_Reduce (NTPItmp, NTPI_a, 6, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)
         CALL MPI_Reduce (NMCBPtmp, NMCBP_a, 1, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)
         CALL MPI_Reduce (NCOREtmp, NCORE_a, 1, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)
      ENDIF

      IF (LVP)
     &   CALL MPI_Reduce (NVPItmp, NVPI_a, 1, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)

      IF (LNMS)
     &   CALL MPI_Reduce (NKEItmp, NKEI_a, 1, I_MPI, MPI_SUM, 0,
     &                                 MPI_COMM_WORLD, ierr)

      IF (LSMS)
     &   CALL MPI_Reduce (NVINTItmp, NVINTI_a, 1, I_MPI, MPI_SUM,
