...
*   Ensure that the indices are in `canonical' order
     ...
     :                   NP(IB),NH(IB),')'            
      ...
      ENDIF
       DO 62 J = NORB, 1, -1
         write(*,*)IC,IR,J,EMTBLOCK(1,J)
   62  continue     

      IBUG1 = 0
      WRITE(*,*) 'the two-electron Coulomb operator and the mass 
     : polarisation In matrixblock12 IC,IR',IC,IR
...
!              LABEL(4,I) = J + NTYPE(3,IC) - 1 
              ...
              ELSE IF (LABEL(4,I) .GE. NTYPE(3,IC)) THEN
                LABEL(4,I) = J + NTYPE(3,IC) - 1
              ELSE
                WRITE(*,*)'STOP IN MATRIXBLOCK12.F'
                STOP
              END IF
            END IF
            IF (LSMS) THEN
   ...
   72  continue     
      ...
      WRITE(*,*) 'the two-electron transverse interaction operator 
     : in matrixblock12 IC,IR',IC,IR
...
!        write(*,*)NORB,NTYPE(4,IC),NTYPE(3,IC)
!        write(*,*)(NP(m),m=NTYPE(3,IC),NTYPE(4,IC),1)
!        write(*,*)(NH(m),m=NTYPE(3,IC),NTYPE(4,IC),1)
          ...
          IF (DABS (COEFF(I)) .GT. CUTOFF) THEN
            ...
            DO 11 J = NORB, 1, -1
              ...
              IF(J .NE. NORB) THEN
                ...
                ELSE IF (LABEL(4,I) .GE. NTYPE(3,IC)) THEN
                  LABEL(4,I) = J + NTYPE(3,IC) - 1 
!                  write(*,*)LABEL(4,I),J,NTYPE(3,IC)
!                  write(*,*)NP(LABEL(4,I)),NH(LABEL(4,I))
                ELSE IF (LABEL(1,I) .GE. NTYPE(3,IC)) THEN
                  LABEL(1,I) = J + NTYPE(3,IC) - 1
                ELSE IF (LABEL(2,I) .GE. NTYPE(3,IC)) THEN
