...
cAK    IATJPO,IASPAR must be INTEGER*4 for compatibility with ifort -i8
      INTEGER*4 IATJPO,IASPAR
...
*=======================================================================
      ...
      CALL alloc (pncfpast, nblock, ISIZE)
      CALL alloc (pncminpast, nblock, ISIZE)
      CALL alloc (pnevecpast, nblock, ISIZE)
...
*=======================================================================
      ...
      NXDIM = 0
      NYDIM = 0
cAK    Must initialize DA,XA,YA,... here since MPI routines rely on their existence in all processes
...
!        print *, "test print: ", WTAEV,WTAEV0,DAMPMX, myid
         IF(DABS(WTAEV-WTAEV0).LT.1.0D-8.and.
...
cAK      IF (NDDIM .GT. 0) THEN
         CALL dalloc (pntrda)
         CALL dalloc (pntnda)
cAK      ENDIF

cAK      IF (NXDIM .GT. 0) THEN
         CALL dalloc (pntrxa)
         CALL dalloc (pntnxa)
cAK      ENDIF

cAK      IF (NYDIM .GT. 0) THEN
         CALL dalloc (pntrya)
         CALL dalloc (pntnya)
cAK      ENDIF
