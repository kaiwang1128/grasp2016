*-----------------------------------------------------------------------
*
*     RPCFGENERATE_S -- A PROGRAM FOR GENERATING PAIR CORRELATION
*                     FUNCTION EXPANSIONS      
*
*     Per Jönsson
*     Malmö University
...
*
*     A selected number of PCFs can be concatenatedto a file concat.f     
*
*     September 2016
*
*-----------------------------------------------------------------------
*
      program rpcfgenerate_s
     ...
     :  '10h-','10h ','10i-','10i ','10k-','10k ','10l-','10l '/
      ...
      write(*,*)
      write(*,*) ' Welcome to the RPCFGENERATE_S program'
      write(*,*) 
      ...
      write(*,*) ' into PCFs. In the current version all outer       '
      write(*,*) ' valence orbitals are treated together.            '
      write(*,*) ' In addition the PCFs from the core are separated  '
      write(*,*) ' in pure S and pure D                              '
...
* --- Open relativistic PCFs involving S excitations from the core

      do i = 1,ncore
        name = trim(adjustl(core(i)))//trim(label)//'.c'
        open(unit=30000+i,file=name,status='unknown')
      end do
...
* --- For j = k two cases dependent on occupation

          if (j.eq.k) then
            if (qorbcsf(j).eq.(qorb(j)-1)) then            
              write(30000+j,'(a)') trim(line1)
              write(30000+j,'(a)') trim(line2)
              write(30000+j,'(a)') trim(line3)
            else
...
* --- Non-relativistic PCFs involving S excitation from core in append 
*     mode
...
C --- Non-relativistic PCFs involving D excitation from core in append
C     mode
