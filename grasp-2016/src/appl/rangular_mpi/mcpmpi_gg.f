************************************************************************
*                                                                      *
      SUBROUTINE mcpmpi (nb, RESTRT, myid, nprocs, fhead)
*                                                                      *
...
*   integration                           Last revision: 01 April 2012 *
*                                                                      *
************************************************************************
*
...
CGG      EXTERNAL COR, CORD, outsdampi
      EXTERNAL CORD, outsdampi
...
! In order for serial and parallel programs to give the same result
      ...
      PARAMETER (CUTOFF = 0.D0)              ! cutoff criterion
!-----------------------------------------------------------------------
!xhh - swap not used
...
*
     ...
     &         WRITE (30) JA, JB, npos
    4    CONTINUE
         if (mod(ja-1,100).eq.0) then   
            PRINT '(A4,I7,A9,I10,A9,I3,A8,I3)', 'Row ', JA,
     &              ' nnonz = ', npos,
     &              ' block = ', nb,' myid = ', myid
!         IF (JA .LE. 2*nprocs .OR. JA .GE. NCF-2*nprocs 
!     &       .OR. MOD (JA-1,5*nprocs) .EQ. myid) THEN
!            PRINT '(A4,I6,A9,I10,A9,I3,A6,I3)', 'Row ', JA, 
!     &              ' nnonz = ', npos,
!     &              ' block = ', nb,' id = ', myid
         ENDIF
    5 CONTINUE
