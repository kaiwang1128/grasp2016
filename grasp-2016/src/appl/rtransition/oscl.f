...
*
      LSAME = trim(name(1)).eq.trim(name(2))
...
c             M = LEVFF+NVECII*(LEVII-1)
              ...
              ! Make it possible to change current transition energy -- added by Jon Grumer 2013-06-07
C              ANSW = 'n'
C              WRITE(*,*) ' Current transition energy [a.u]  = ', ET(M)
C              WRITE(*,*)
C10            WRITE(*,*) ' Do you want to change this energy (y/n)?'
C              READ(*,'(A1)') ANSW
C              IF(ANSW.EQ.'N'.OR.ANSW.EQ.'n'.OR.ANSW.EQ.'Y'.OR.
C     :          ANSW.EQ.'y') THEN
C                IF(ANSW.EQ.'Y'.OR.ANSW.EQ.'y') THEN
C                  WRITE(*,*) ' Give new transition energy [a.u]:'
C                  READ(*,'(1D20.7)') ET(M)
C                  WRITE(*,*) 'New energy is [a.u] =', ET(M)
C                ELSEIF(ANSW.EQ.'N'.OR.ANSW.EQ.'n') THEN
C                  WRITE(*,*) 'Transition energy is unchanged.'
C                END IF
C              ELSE
C                WRITE(*,*) 'Please answer in format Y/y/N/n, redo!'
C                GOTO 10
C              ENDIF
