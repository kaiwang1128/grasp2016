...
CFF       IF (MOD(JA,20).EQ.0.AND.IK.EQ.1) THEN 
          IF (MOD(JA,100).EQ.0.AND.IK.EQ.1) THEN 
            WRITE(*,*) ' JA1 =',JA,JA-NCF0+1
CFF       ELSEIF (MOD(JA,20).EQ.0.AND.IK.EQ.2) THEN
          ELSEIF (MOD(JA,100).EQ.0.AND.IK.EQ.2) THEN
...
*
            IF (COMP) THEN
CGG            CALL ONEPARTICLEJJ(KA,IOPAR,JA,JB,IA,IB,TSHELL)
