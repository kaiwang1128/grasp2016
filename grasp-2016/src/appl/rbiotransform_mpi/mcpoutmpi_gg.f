...
c      DATA KSTART/0/
c      SAVE KSTART,EVSC,NCFSC,NVECSC
...
*
        DO JA = myid+NCF0,NCFBLK(IBLK),nprocs
c       DO JA = myid+1,NCF,nprocs
*R      DO JA = 1,NCF
...
*
            ...
            IF (COMP) THEN
              CALL ONEPARTICLEJJ(KA,IOPAR,JA,JB,IA,IB,TSHELL)
...
*
      DO L = 1,KAMAX
        IF (LLISTT(L).GT.0) THEN
            CALL QQSORT (L,LLISTT(L),IK,NAME,KAMAX,IBLK)
          ELSE
