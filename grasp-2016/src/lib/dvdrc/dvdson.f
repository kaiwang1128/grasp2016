...
c     :			 HIEND,NLOOPS,IERR)      
        ...
        integer*8 Ni8,iBasis,ieigval,iAB,iS,itempS,iSvec,iscra1,ioldval
...
*       ..Check for enough workspace for Dvdson
	IF ((IWRSZ.LT.(LIM*(2*Ni8+LIM+(NUME+10))+NUME+NOC*Ni8)).OR.
...
*
        ...
        !endif
	iOrtho    =1
	iBasis    =iOrtho  + Ni8*NOC
	ieigval   =iBasis  + Ni8*LIM
	iAB	  =ieigval + LIM
	iS	  =iAB     + Ni8*LIM
...
CCC	iscra1    =iSvec   + LIM*(NUME+1)
	...
	ioldval   =iscra1  + 8*LIM
        ...
        !if (iBasis .lt. 0) stop "Error, iBasis .lt. 0"
        !if (ieigval.lt. 0) stop "Error, ieigval.lt. 0"
        !if (   iAB .lt. 0) stop "Error,    iAB .lt. 0"
        !if (    iS .lt. 0) then
        !   print *, "N  =", N
        !   print *, "LIM=", LIM
        !   print *, "iAB=", iAB
        ...
        !   stop "Error,     iS .lt. 0"
        !endif
        !if (itempS .lt. 0) stop "Error, itempS .lt. 0"
        !if ( iSvec .lt. 0) stop "Error,  iSvec .lt. 0"
        !if (iscral .lt. 0) stop "Error, iscral .lt. 0"
        !if (ioldval.lt. 0) stop "Error, ioldval.lt. 0"
...
*
	...
	iIcv      =iscra3  +LIM
        ...
        if (iscra3 .lt. 0) stop "Error, iscra3 .lt. 0"
        if (  iIcv .lt. 0) stop "Error,   iIcv .lt. 0"
*
* Initialize tha basis, the AB, the S. 
*
	
 100	call initdvd(IRC,IREV,Ni8,NOC,NIV,NUME+1,LIM,HIEND,WORK(iscra1),
...
*
 ...
 200	CALL DVDRVR(IRC,IREV,Ni8,HIEND,LIM,MBLOCK,NOC,
...
*
	CALL DCOPY(NUME,WORK(ieigval),1,WORK(iBasis+Ni8*NUME),1)
	CALL DCOPY(NUME,WORK(ioldval),1,WORK(iBasis+(Ni8+1)*NUME),1)
	CALL DCOPY(NUME,WORK(iscra1),1,WORK(iBasis+(Ni8+2)*NUME),1)
*
...
*
	...
	END
*=======================================================================
	SUBROUTINE ADDS(Ni8,LIM,KPASS,NNCV,BASIS,AB,S)
*=======================================================================
*	Called by: DVDSON
*
...
*-----------------------------------------------------------------------
	IMPLICIT REAL*8          (A-H,O-Z)
        integer*8 Ni8
	DIMENSION BASIS(Ni8*LIM),AB(Ni8*LIM)
...
*
 ...
 20	CONTINUE
	...
	END
*=======================================================================
	SUBROUTINE DVDRVR(IRC,IREV,Ni8,HIEND,LIM,MBLOCK,NOC,
     :			  NUME,NIV,NEIG,ISELEC,
     :			  CRITE,CRITC,CRITR,MAXITER,
...
CCC	DIMENSION SVEC(LIM*(NUME+1)),EIGVAL(LIM)
	...
	DIMENSION BASIS(Ni8*LIM),OrthoBasis(Ni8*LIM+NOC*Ni8),AB(Ni8*LIM)
...
!23456789012345678901234567890123456789012345678901234567890123456789012
              CALL OVFLOW(Ni8,rest,KPASS,SCRA1,BASIS,AB,S,SVEC,EIGVAL)
*             CALL OVFLOW(Ni8,NUME,KPASS,SCRA1,BASIS,AB,S,SVEC,EIGVAL)
	   END IF
*
* Compute and add the new residuals. NNCV is set to the number of new
* vectors that have not converged. If none, DONE=true, exit.
*
 	   CALL NEWVEC(Ni8,NUME,LIM,MBLOCK,KPASS,CRITR,NNCV,INCV,
...
*
	   NEWSTART = KPASS*Ni8+1

	   call mgs_nrm(Ni8,noc+kpass,nncv,scra1(LIM+1),OrthoBasis)
*-----------------------------------------------------------------------
...
*
	   IF (HIEND) CALL DSCALi8(Ni8*NNCV,-1.D0,AB(NEWSTART),1)
*
* Add new column in S, from the NNCV new vectors.
*
	   CALL ADDS(Ni8,LIM,KPASS,NNCV,BASIS,AB,S)
...
*
 ...
 40	   OLDVAL(I) = ABS(OLDVAL(I)-EIGVAL(I))
	...
	CALL MULTBC(Ni8,KPASS,NUME,SVEC,SCRA1,BASIS)
	CALL MULTBC(Ni8,KPASS,NUME,SVEC,SCRA1,AB)
*
* i=1,NUME residual(i)= DCi-liBCi= newDi-linewBi
* temporarily stored in AB(NUME*N+1)
*
	DO 50 I = 1,NUME
	   CALL DCOPY(N,AB((I-1)*Ni8+1),1,AB(NUME*Ni8+1),1)
	   CALL DAXPY(N,-EIGVAL(I),BASIS((I-1)*Ni8+1),1,AB(NUME*Ni8+1),1)
	   SCRA1(I) = DDOT(N,AB(NUME*Ni8+1),1,AB(NUME*Ni8+1),1)
...
*
 ...
 60     IRC = 0
	...
	END
*=======================================================================
        subroutine initdvd(IRC,IREV,Ni8,NOC,NIV,NUME,LIM,HIEND,SCRA1,
...
*-----------------------------------------------------------------------
	...
	dimension IREV(*)
        integer*8 IREV
	dimension OrthoBasis(Ni8*(NOC+LIM)),Basis(Ni8*LIM),AB(Ni8*LIM)
	dimension S(*),SCRA1(*)
	logical HIEND
...
*       ----------------------------------------------------------------
	...
	if (iend.lt.NUME) then
	   new = min( nume-iend, iend-ist+1)
           call dcopyi8(Ni8*new,AB((ist-1)*Ni8+1),1,Basis(1+iend*Ni8),1)
*            ..orthonormalize OrthoBasis i.e.,
*            B-1..B-noc,B1,...,Biend,( Biend+1,...Biend+new )
	   call mgs_nrm(Ni8,noc+iend,new,scra1,OrthoBasis)
	   ist  = iend + 1
           iend = iend+new
	   goto 10
	endif
...
*
        IF (HIEND) CALL DSCALi8(Ni8*NIV,-1.D0,AB,1)
*
* Also find the small matrix S = B^TAB.
*
        KPASS = 0
        CALL ADDS(Ni8,LIM,KPASS,NIV,Basis,AB,S)
...
*=======================================================================
        SUBROUTINE MULTBC(Ni8,K,M,C,TEMP,B)
*=======================================================================
*	called by: DVDRVR
*
...
*-----------------------------------------------------------------------
        IMPLICIT REAL*8          (A-H,O-Z)
        integer*8 Ni8
	DIMENSION B(Ni8*K),C(K*M),TEMP(M)
*-----------------------------------------------------------------------
        N = Ni8
        DO 10 IROW=1,N
  ...
  10    CONTINUE
        ...
        END
*=======================================================================
	SUBROUTINE NEWVEC(Ni8,NUME,LIM,MBLOCK,KPASS,CRITR,NNCV,INCV,
...
*	      ..More to be added in the block
 ...
 20	NNCV=NADDED
	...
	END
*=======================================================================
	SUBROUTINE OVFLOW(Ni8,NUME,KPASS,SCRA1,BASIS,AB,S,SVEC,EIGVAL)
*=======================================================================
*	Called by: DVDRVR
...
*-----------------------------------------------------------------------
	IMPLICIT REAL*8          (A-H,O-Z)
        ...
        DIMENSION EIGVAL(KPASS),BASIS(Ni8*KPASS),AB(Ni8*KPASS)
*-----------------------------------------------------------------------
*   on entry
*   -------
...
*=======================================================================
!           call mgs_nrm(Ni8,noc+iend,new,scra1,OrthoBasis)
	subroutine mgs_nrm(Ni8,kp,new,scra,B)
*=======================================================================
...
*-----------------------------------------------------------------------
	implicit REAL*8          (a-h,o-z)
        integer*8 Ni8,newstart,jcur,kcur
	dimension B((kp+new)*Ni8),scra(new)
      !INCLUDE 'mpif.h'
...
!     :                 MPI_COMM_WORLD,ierr)
	...
	return
      ...
      subroutine  dcopyi8(n,dx,incx,dy,incy)
c
...
c
   ...
   50 continue
      ...
      subroutine  dscali8(n,da,dx,incx)
c
